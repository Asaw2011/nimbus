...
export async function saveRound(round: Round): Promise<void> {
  await saveRoundJson(round.id, JSON.stringify(round));
}

/** Write an already-serialized round. The hot autosave path uses this so the
 *  round is walked ONCE (stringify) instead of twice ($state.snapshot deep-clone
 *  and then stringify) on every flush. */
export async function saveRoundJson(id: string, json: string): Promise<void> {
  if (inTauri()) {
    await invoke("save_round", { id, json });
  } else {
