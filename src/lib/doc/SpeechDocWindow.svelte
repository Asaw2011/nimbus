<script lang="ts">
  ...
  onMount(async () => {
    initial = await loadDocContent(docId);
    ready = true;
  ...
  function onDocChange() {
    // Serialize once when the debounce fires, not per keystroke.
  ...
  async function sendToSpeechCursor(nodes: unknown[]) {
    if (!nodes.length) return;
    const { emit } = await import("@tauri-apps/api/event");
    await emit("nimbus:send-to-speech", { nodes, atCursor: true });
  }
</script>
...
<div class="doc-window">
  ...
  {#if ready}
    <div class="window-body">
      <SpeechDoc bind:this={docRef} initialDoc={initial} {docName} onchange={onDocChange} poppedOut onpopout={dockBack} onTilde={sendToSpeech} onTildeCursor={sendToSpeechCursor} />
    </div>
  {/if}
