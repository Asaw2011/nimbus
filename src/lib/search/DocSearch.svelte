<script lang="ts">
  ...
  // Parsed trees of docs you've dived into. Capped: a card file's tree can be
  // tens of megabytes, and browsing a dozen of them used to keep every one of
  // them alive for the rest of the session.
  const PARSE_CACHE_MAX = 6;
  const parseCache = new Map<string, DocNode[]>();

  /** Cache read that also marks the entry most-recently-used. */
  function cachedParse(path: string): DocNode[] | undefined {
  ...
  function cacheParse(path: string, nodes: DocNode[]) {
    parseCache.set(path, nodes);
  ...
  // Brief "inserted ✓" confirmation, since the panel now stays open on insert.
  let insertedMsg = $state("");
  let insertedTimer: ReturnType<typeof setTimeout> | null = null;
  function flashInserted(name: string) {
    insertedMsg = `Inserted “${name.slice(0, 40)}”`;
  ...
  function onWindowPointerDown(e: PointerEvent) {
    ...
    if (!t) return;
    if (t.closest(".ds-panel, [data-r], .doc-pane, .ProseMirror, .cm-editor")) return;
    onclose();
  }
  ...
  async function diveInto(file: LibFile, keepQuery = false) {
    const cached = cachedParse(file.path);
    if (cached) {
      setDived(file, cached, keepQuery);
      return;
    }
  ...
  function activateSelected() {
    if (mode === "files") {
      ...
      // Stay open after inserting so you can keep pulling blocks; ✕ / Esc /
      // click-off closes it. Your place in the list is kept.
      if (r) { void insertNode(r.node); flashInserted(r.node.text); }
    }
  }
...
<div class="ds-panel" style="transform: translate({px}px, {py}px)" onkeydown={onkeydown} onpointermove={moveDrag} onpointerup={endDrag} role="dialog" aria-label="Doc Search" tabindex="-1">
  ...
  <div class="ds-header" onpointerdown={startDrag}>
    <div class="ds-title">
      ...
      <span class="ds-hint">{insertedMsg || "· drag to move"}</span>
    </div>
    ...
    {:else}
      ...
      {:else}
        {#each rows as r, i (r.key)}
          {@const ci = r.node.isAnalytic ? { label: "ANL", cls: "c-anl" } : chipFor(r.node.level)}
          <!-- svelte-ignore a11y_click_events_have_key_events -->
          <div class="ds-row" class:sel={i === selectedIdx}
            ...
            onclick={() => { selectedIdx = i; void insertNode(r.node); flashInserted(r.node.text); }}
            onmouseenter={() => (selectedIdx = i)}>
            {#if r.hasKids}
