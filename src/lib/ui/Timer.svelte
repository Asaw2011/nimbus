<script lang="ts">
  ...
  // presets (edited in Settings). Draggable; beeps when a countdown hits zero.
  ...
  function startDrag(e: PointerEvent) {
    ...
    const move = (ev: PointerEvent) => { pos = { x: ev.clientX - offX, y: ev.clientY - offY }; };
...
>
  ...
  <div class="t-head" onpointerdown={startDrag}>
    ...
    <span class="t-mode">{mode === "stopwatch" ? "Stopwatch" : (activePreset !== null ? settings.timerPresets[activePreset]?.label : "Countdown")}</span>
...
<style>
  ...
  .t-display.finished { color: #fff; background: var(--mark-dropped, #c0392b); animation: flash 0.6s steps(1) infinite; }
