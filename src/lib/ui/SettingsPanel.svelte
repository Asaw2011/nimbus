...
>
  ...
  >
    ...
    <section>
      ...
      <label class="row">
        <span>Compact doc chrome<br /><small class="sub">Tighter speech-doc toolbar + outline, so the document gets more room</small></span>
        <input
          type="checkbox"
  ...
  flow.down();
    ...
    <section>
      ...
      <div class="readers">
        {#each settings.readers as r, i (i)}
          <div class="reader-row">
            <input
              class="reader-name"
              value={r.name}
              placeholder="Reader name"
              oninput={(e) => { settings.readers[i].name = e.currentTarget.value; settings.save(); }}
            />
            <input
              class="reader-wpm"
              type="number" min="1" max="1200"
              value={r.wpm}
              oninput={(e) => { settings.readers[i].wpm = Math.max(1, Math.round(Number(e.currentTarget.value)) || 1); settings.save(); }}
            />
            <span class="reader-unit">wpm</span>
            <button class="reader-del" title="Remove this reader" aria-label="Remove reader"
              onclick={() => { settings.readers = settings.readers.filter((_, j) => j !== i); settings.save(); }}>×</button>
          </div>
        {/each}
      </div>
      <button class="add-reader"
        onclick={() => { settings.readers = [...settings.readers, { name: `Reader ${settings.readers.length + 1}`, wpm: 200 }]; settings.save(); }}
      >+ Add reader</button>
    </section>
    <section>
      <h3>Timer presets</h3>
      ...
      <div class="readers">
        {#each settings.timerPresets as p, i (i)}
          <div class="reader-row">
            ...
            <input
              class="reader-wpm"
              value={`${Math.floor(p.seconds / 60)}:${(p.seconds % 60).toString().padStart(2, "0")}`}
              placeholder="m:ss"
              onchange={(e) => {
...
<style>
  ...
  .readers { display: flex; flex-direction: column; gap: 6px; margin-bottom: 8px; }
  .reader-row { display: flex; align-items: center; gap: 6px; }
  .reader-name {
  ...
  .reader-unit { font-size: 11px; color: var(--text-dim); }
  .reader-del {
    border: none; background: transparent; color: var(--text-dim);
    font-size: 16px; line-height: 1; cursor: pointer; padding: 2px 6px;
  }
  .reader-del:hover { color: var(--mark-dropped, #c0392b); }
  .add-reader {
  ...
  .add-reader:hover { color: var(--text); border-color: var(--accent); }
  .row {
    display: flex;
    justify-content: space-between;
    align-items: center;
