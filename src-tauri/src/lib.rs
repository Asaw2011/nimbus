...
pub fn run() {
    tauri::Builder::default()
        ...
        // Start/pause and reset for the POPPED-OUT timer while another app
        // (CardMirror) has focus. The timer window registers its shortcuts only
        // while it is open, so they are never held otherwise.
